...
func TestNormalizeCLIArgsAllowsFlagsAfterCommands(t *testing.T) {
	tests := []struct {
		args []string
		want []string
	}{
	...
	for _, tt := range tests {
		got, err := normalizeCLIArgs(tt.args)
		if err != nil {
			t.Fatalf("normalizeCLIArgs(%v): %v", tt.args, err)
		}
		if strings.Join(got, "|") != strings.Join(tt.want, "|") {
			t.Errorf("normalizeCLIArgs(%v) = %v, want %v", tt.args, got, tt.want)
		}
	}
}
...
func replaceHooks(t *testing.T) func() {
	...
	oldADBDevices := adbDevices
	oldADBDisconnect := adbDisconnect
	oldADBPair := adbPair
	...
	adbDevices = func(context.Context, string) ([]adb.DeviceEntry, error) { return nil, nil }
	adbDisconnect = func(context.Context, string, string) (string, error) { return "", nil }
	...
	return func() {
		...
		adbDevices = oldADBDevices
		adbDisconnect = oldADBDisconnect
		adbPair = oldADBPair
