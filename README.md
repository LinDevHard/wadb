...
4. After pairing succeeds, the phone advertises `_adb-tls-connect._tcp`. `wadb` tries connect endpoints from the same IP as the pairing announce first, then falls back to other discovered endpoints. If its own browse turns up nothing, it asks `adb mdns services` — adb runs a separate mDNS implementation and often has the announce cached already.
